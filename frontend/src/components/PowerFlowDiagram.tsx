...
interface Props {
  ...
  load1Data:   ACData | null;
  load2Data:   ACData | null;
  load3Data:   ACData | null;
}

// ─── SOC trend — determines charging from SOC direction ──────────────────────
...
function SolarNode({ data }: NodeProps) {
  ...
  return (
    <div style={{ width: 152, background: "#111827", border: `2px solid ${SOLAR_COLOR}`, borderRadius: 12, padding: 14, userSelect: "none" }}>
      <Handle type="source" position={Position.Bottom} id="solar-out"
...
function BatteryNode({ data }: NodeProps) {
  ...
  return (
    <div style={{ width: 172, background: "#111827", border: `2px solid ${BAT_COLOR}`, borderRadius: 12, padding: 16, userSelect: "none" }}>
      <Handle type="target" position={Position.Top} id="bat-top-in"
        style={{ background: BAT_COLOR, border: "2px solid #0b0f1a", width: 10, height: 10 }} />
      <Handle type="source" position={Position.Right} id="bat-out"
        style={{ background: BAT_COLOR, border: "2px solid #0b0f1a", width: 10, height: 10 }} />
      <Handle type="target" position={Position.Right} id="bat-in"
        style={{ background: BAT_COLOR, border: "2px solid #0b0f1a", width: 10, height: 10 }} />
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
        ...
        <span style={{ fontSize: 11, fontWeight: 700, color: BAT_COLOR, letterSpacing: "0.08em", textTransform: "uppercase" }}>Battery</span>
      </div>
      ...
      <p style={{ fontSize: 10, color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 12 }}>State of Charge</p>
...
function BusNode({ data }: NodeProps) {
  ...
  return (
    <div style={{ width: 110, background: "#111827", border: "1px solid #1e293b", borderRadius: 12, padding: 16, userSelect: "none" }}>
      ...
      <p style={{ fontSize: 10, fontWeight: 600, color: BUS_COLOR, textTransform: "uppercase", letterSpacing: "0.08em", textAlign: "center", marginBottom: 8 }}>Bus</p>
...
function LoadNode({ data }: NodeProps) {
  const { label, color, voltage, current, power } = data as {
    label: string; color: string; voltage?: number; current?: number; power?: number;
  };
  return (
    <div style={{ width: 172, background: "#111827", border: "1px solid #1e293b", borderTop: `2px solid ${color}`, borderRadius: 12, padding: 16, userSelect: "none" }}>
...
// ─── Node types (outside component to prevent re-registration) ────────────────
...
const INITIAL_NODES = [
  { id: "solar",   type: "solarNode",   position: { x: 60,  y: -20  }, data: {} },
  { id: "battery", type: "batteryNode", position: { x: 50,  y: 180  }, data: {} },
  { id: "bus",     type: "busNode",     position: { x: 330, y: 250  }, data: {} },
  { id: "load1",   type: "loadNode",    position: { x: 560, y: 110  }, data: { label: "Load Group 1", color: LOAD_COLORS.load1 } },
  { id: "load2",   type: "loadNode",    position: { x: 560, y: 270  }, data: { label: "Load Group 2", color: LOAD_COLORS.load2 } },
  { id: "load3",   type: "loadNode",    position: { x: 560, y: 430  }, data: { label: "Load Group 3", color: LOAD_COLORS.load3 } },
];
...
export default function PowerFlowDiagram({ batteryData, load1Data, load2Data, load3Data }: Props) {
  const [nodes, setNodes, onNodesChange] = useNodesState(INITIAL_NODES);
  const [edges, setEdges, onEdgesChange] = useEdgesState([]);

  const isCharging = useSocTrend(batteryData?.soc);

  const totalPower  = (load1Data?.active_power ?? 0) + (load2Data?.active_power ?? 0) + (load3Data?.active_power ?? 0);
  const batPowerAbs = Math.abs(batteryData?.battery_power ?? 0);
      ...
      switch (node.id) {
        ...
        case "battery":
          return { ...node, data: {
            ...
            power:   batteryData?.battery_power,
            soc:     batteryData?.soc,
            isCharging,
          }};
        case "bus":
        ...
        case "load1":
          return { ...node, data: { ...node.data, voltage: load1Data?.ac_voltage, current: load1Data?.ac_current, power: load1Data?.active_power } };
        case "load2":
          return { ...node, data: { ...node.data, voltage: load2Data?.ac_voltage, current: load2Data?.ac_current, power: load2Data?.active_power } };
        case "load3":
          return { ...node, data: { ...node.data, voltage: load3Data?.ac_voltage, current: load3Data?.ac_current, power: load3Data?.active_power } };
        default:
          return node;
      }
    }));
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [batteryData, load1Data, load2Data, load3Data, isCharging]);

  useEffect(() => {
    const edgeWidth = (p: number) => Math.min(6, Math.max(2, 2 + (Math.abs(p) / MAX_POWER_REF) * 4));

    const batBusEdge = isCharging
      ? { id: "bat-bus", source: "bus", sourceHandle: "bus-bat-out", target: "battery", targetHandle: "bat-in" }
      : { id: "bat-bus", source: "battery", sourceHandle: "bat-out", target: "bus", targetHandle: "bus-bat-in" };
    ...
    setEdges([
      {
        ...
        animated: isCharging,
        style: { stroke: isCharging ? SOLAR_COLOR : "#1e293b", strokeWidth: isCharging ? edgeWidth(solarPower) : 1.5, opacity: isCharging ? 1 : 0.3 },
      },
      {
        ...batBusEdge,
        animated: true,
      ...
      {
        id: "bus-l1", source: "bus", sourceHandle: "right-1", target: "load1", targetHandle: "left",
        animated: true,
        style: { stroke: LOAD_COLORS.load1, strokeWidth: edgeWidth(load1Data?.active_power ?? 0) },
      },
      {
        id: "bus-l2", source: "bus", sourceHandle: "right-2", target: "load2", targetHandle: "left",
        animated: true,
        style: { stroke: LOAD_COLORS.load2, strokeWidth: edgeWidth(load2Data?.active_power ?? 0) },
      },
      {
        id: "bus-l3", source: "bus", sourceHandle: "right-3", target: "load3", targetHandle: "left",
        animated: true,
        style: { stroke: LOAD_COLORS.load3, strokeWidth: edgeWidth(load3Data?.active_power ?? 0) },
      },
    ]);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [batteryData, load1Data, load2Data, load3Data, isCharging]);
  ...
  return (
    <div style={{ width: "100%", height: 520, background: "#0b0f1a", borderRadius: 12, overflow: "hidden", border: "1px solid #1e293b" }}>
      <ReactFlow
        nodes={nodes}
        edges={edges}
